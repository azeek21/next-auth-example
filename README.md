...
# callbacks (adding custom properties to session)
PROBLEM: now if you check the session, there's no unique identifier for the user that we can make use of to do CRUD operations for this specific user. Or you might yourself need to add some custom properties like phone number or something. <br/>

SOLUTION: To solve above problem, nextauth has callbacks, which will be called before issiung a session. We can declare and use callbacks to add custom properties with these callbacks. <br/>


HOWT: 
1. inside `./src/pages/api/[...nextauth].ts` add another property object called `callbacks` inside `AuthOptions` of `NextAuth()` 
2. In `callbacks` declare 2 `asynchronous` functions, which are `jwt` and `session` <br/>
Example: `./src/api/auth/[...nextauth].ts`
```
export default NextAuth({
    ...,
...
```
3. Now wherever we call `useSession()` or `getSession()` an `id` field will be awailable inside session object.
